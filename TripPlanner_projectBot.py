...
def get_token():
    ...
    if not token:
        raise ValueError(
            "❌ Токен не найден в .env файле. Создайте файл .env с содержимым: TOKEN=ваш_токен"
        )
...
def send_recommendation(message):
    try:
        ...
        if len(parts) < 3:
            bot.send_message(
                message.chat.id, "❌ Используйте: /recommend [город] [месяц]"
            )
            return
    ...
    if text == "🏙️ Список городов":
        ...
        try:
            ...
            fake_message = type(
                "obj",
                (object,),
                {"text": f"/recommend {city} {month}", "chat": message.chat},
            )
            send_recommendation(fake_message)
        except:
            bot.send_message(
                message.chat.id, "❌ Не понимаю. Используйте команды из меню."
            )
